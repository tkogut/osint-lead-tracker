# Plan 042: Integracja nowego skrapera biznes-polska.pl

Ten plan opisuje wdrożenie nowej wtyczki skrapera dla portalu **biznes-polska.pl** z uwzględnieniem dwufazowego pobierania danych (Phase 1: szybkie skanowanie bez logowania via `curl_cffi` + Phase 2: autoryzowane wyciąganie danych kontaktowych via `Playwright` dla dopasowanych leadów).

---

## Cel wdrożenia
Dodanie obsługi serwisu `biznes-polska.pl` jako dedykowanego źródła OSINT:
1. Pobieranie listy ogłoszeń z `https://www.biznes-polska.pl/przetargi/` za pomocą `curl_cffi`.
2. Dopasowanie ogłoszeń na podstawie słów kluczowych (pre-filter).
3. Logowanie i pobieranie danych kontaktowych za pomocą jednej sesji `Playwright` dla pasujących ogłoszeń.
4. Możliwość weryfikacji danych uwierzytelniających w ustawieniach (Test autoryzacji).
5. Pełna integracja z piaskownicą (Sandbox) w panelu WWW.

---

## Proponowane Zmiany

### 1. Rozbudowa Playwright Fetcher
#### [MODIFY] [playwright_fetcher.py](file:///home/tkogut/projects/osint-lead-tracker/src/scrapers/playwright_fetcher.py)
Dodamy obsługę autoryzacji dla kontekstu `"BiznesPolska"` w funkcji `_login_session`:
...
### 2. Nowa wtyczka skrapera
#### [NEW] [biznes_polska.py](file:///home/tkogut/projects/osint-lead-tracker/src/scrapers/biznes_polska.py)
Tworzymy dedykowaną wtyczkę skrapera dla `biznes-polska.pl`:
                ...
                try:
                    ...
                    for detail_url in unvisited_urls:
                        ...
                        try:
                            ...
                            # Szukamy klasy col-added-date lub tekstu w kodzie
                            pub_date_str = None
...
### 4. Integracja z panelem Settings i Sandboxem
#### [MODIFY] [main.py](file:///home/tkogut/projects/osint-lead-tracker/src/main.py)
1. W `/api/sources` zarejestrujemy BiznesPolska:
```python
...
2. W `/api/settings/verify-credentials` obsłużymy weryfikację logowania dla BiznesPolska:
```python
    elif scraper == "BiznesPolska":
        logger.info("Weryfikacja danych logowania dla BiznesPolska...")
        async with AsyncSession(impersonate="chrome124") as s:
            r = await s.post("https://www.biznes-polska.pl/logowanie/", data={
                "username": username,
                "password": password,
                "rememberme": "y"
            }, timeout=15)
            ...
            return {"success": True, "message": "Dane uwierzytelniające dla BiznesPolska są poprawne."}
```

3. W `sandbox_fetch_url()` i `run_sandbox_test()` obsłużymy pobieranie z logowaniem w kontekście `"BiznesPolska"`.

#### [MODIFY] [schemas.py](file:///home/tkogut/projects/osint-lead-tracker/src/schemas.py)
Dodamy automatyczne generowanie domyślnych kluczy ustawień dla nowego skrapera: `SCRAPER_BIZNESPOLSKA_USER` oraz `SCRAPER_BIZNESPOLSKA_PASS` (zamaskowane jako `******` w ustawieniach).

#### [MODIFY] [app.js](file:///home/tkogut/projects/osint-lead-tracker/src/static/app.js)
Dodamy nową sekcję w Settings dla BiznesPolska, pozwalającą zarządzać jego loginem i hasłem oraz uruchomić przycisk "Testuj Autoryzację".

#### [MODIFY] [index.html](file:///home/tkogut/projects/osint-lead-tracker/src/static/index.html)
Dodamy `"BiznesPolska"` do listy wyboru scrapera w zakładce Sandbox.
...
### Testy Automatyczne
1. Napiszemy testy jednostkowe `tests/test_biznes_polska.py` symulujące dwufazowy scraper i sprawdzające poprawne filtrowanie, wyciąganie linków i fuzję danych.
2. Uruchomienie testów:
   ```bash
   PYTHONPATH=src .venv/bin/python -m unittest tests/test_biznes_polska.py
   ```

### Weryfikacja Ręczna (z udziałem Użytkownika)
1. Po wdrożeniu zmian na VPS i podaniu poprawnych danych logowania w panelu Ustawień (Settings), użytkownik klika przycisk **"Testuj Autoryzację"** dla BiznesPolska i weryfikuje komunikat sukcesu.
2. W zakładce Sandbox użytkownik wkleja przykładowy link ogłoszenia z biznes-polska.pl, wybiera kontekst `"BiznesPolska (z logowaniem)"` i klika **"Pobierz zawartość"**. Weryfikuje, czy pobrana treść zawiera pełne szczegóły ogłoszenia (w tym dane kontaktowe).
